...
import { getProducts, getLaboratories } from '../services/publicApi';
...
export default function Products() {
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      getProducts({ q: q || undefined, laboratoryId: laboratoryId || undefined, page, pageSize: 9 })
        .then(setData)
        .catch(() => setError(true));
    }, 300);
  ...
  return (
    <div className="section container-page">
      <SEO title={t('testProgram.title')} />
      <Breadcrumb items={[{ label: t('testProgram.title') }]} />
      <h1 className="mt-4 text-3xl md:text-4xl font-extrabold text-primary">{t('testProgram.title')}</h1>
